...
logger = test_init("test.25.sql")
logger_s = test_init("test.25.sql.server")
...
try:
	os.unlink('/tmp/test25.db')
except EnvironmentError:
	pass
engine = create_engine('sqlite:////tmp/test25.db', echo=(True if os.environ.get('TRACE',False) else False))
...
class Test25_server(BrokerServer):
	seq = 0
	@cached_property
	def root(self):
...
class Test25_client(TestClient):
	def __init__(self,*a,**k):
		self.a = [None]
		for i in range(3):
			self.a.append(AsyncResult())
		super(Test25_client,self).__init__(*a,**k)
...
class Tester(TestMain):
	client_factory = Test25_client
	server_factory = Test25_server
